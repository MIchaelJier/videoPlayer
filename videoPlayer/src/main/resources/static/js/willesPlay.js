$(function() {
	...
	function ShowInvalidLoginMessage(){
		...
		if(confirm('你已经看了'+maxTime/3600+'小时了,是否休息一下')) {
	...
	playVideo.attr('src','video/'+videosrc+'.mp4'); //chome需要绝对路径，否则currenttime会无效，http://localhost:63342/videoPlayer/
		//获取历史记录中的播放时间
	var historyct = $.getUrlParam('ct');
	if(historyct) {
		playVideo[0].currentTime = parseInt(historyct);
	}
	//关闭页面瞬间保存播放记录
	function closesavemassge(){
		var ct = playVideo[0].currentTime;//获取历史记录的currentTime
		var un=$.cookie('name');
		var vn = $.getUrlParam('dv');
		var dt = new Date;
		$.ajax({
			url:"/savetime",
			data:{
